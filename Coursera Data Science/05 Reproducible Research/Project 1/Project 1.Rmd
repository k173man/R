Project 1 - Activity Data Ananlysis
=====================================
...
###What is mean total number of steps taken per day (excluding `NAs`)
...
```
When excluding observations in which the `steps` variable contains a missing value (`NAs`), the rounded mean daily steps are `r meanDailyStepsXcludNAs`, and the median daily steps are `r medianDailyStepsXcludNAs`
...
```
The `r maxAvgStepsInterval` interval has the maximum number of steps.
...
countOfNAs <- activity %>% 
  ...
  nrow
```
The are `r countOfNAs` rows with missing values.
...
```
With the missing values imputed, the rounded mean daily steps are `r round(meanDailySteps)`, and the median daily steps are `r medianDailySteps`.  These values are very similar to the same values, when computed using the dataset that excluded missing values.
